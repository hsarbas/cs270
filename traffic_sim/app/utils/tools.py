...
def locate_global(road, pos, lane):
    ...
    # Get innermost lane
    edge_x, edge_y = delta_pt_in_perp_line(x0, y0, xt, yt, to_px(road.width) / 2.0)
    ref_x0, ref_y0, ref_xt, ref_yt = x0 - edge_x, y0 - edge_y, xt - edge_x, yt - edge_y

    # Get actual point
    lat_dx, lat_dy = delta_pt_in_perp_line(ref_x0, ref_y0, ref_xt, ref_yt, lane * to_px(road.lane_width))
